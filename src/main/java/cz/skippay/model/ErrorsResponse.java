...
import cz.skippay.model.ErrorsResponseErrorsInner;
...
public class ErrorsResponse {
  ...
  private List<ErrorsResponseErrorsInner> errors = new ArrayList<>();

  public ErrorsResponse() {
  }

  public ErrorsResponse errors(List<ErrorsResponseErrorsInner> errors) {
    
    this.errors = errors;
    return this;
  }

  public ErrorsResponse addErrorsItem(ErrorsResponseErrorsInner errorsItem) {
    if (this.errors == null) {
  ...
  **/
  ...
  public List<ErrorsResponseErrorsInner> getErrors() {
    return errors;
  }


  public void setErrors(List<ErrorsResponseErrorsInner> errors) {
    this.errors = errors;
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to ErrorsResponse
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!ErrorsResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ErrorsResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ErrorsResponse` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ErrorsResponse.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayerrors.size(); i++) {
        ErrorsResponseErrorsInner.validateJsonObject(jsonArrayerrors.get(i).getAsJsonObject());
      };
  }
